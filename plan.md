...
| 3.26 Isolated fixed-strike price ladders | Complete: strict Pyth `closes above K` discovery, independent `price_ladder_*` tables, executable complementary-book bounds, weighted monotonic close distributions, checkpoint cross-market diagnostics, and a localhost-only three-view dashboard. | This sidecar cannot write paper positions or modify the supervisor, Top-5 policy, entry gates, or sizing; `CONFIRM/MIXED/DISAGREE/UNRELIABLE` are research labels only. |
...
| 3.17 Operator visibility | Complete: Rich live terminal dashboard, human-readable supervisor summaries, JSON mode, and SQLite-backed state reduce raw-event inspection. Stable symbol rows show immutable 12:00/14:00/15:30 New York checkpoint direction, fair probability, recorded ask, edge, action and fee-aware limit; the capped paper portfolio includes exact New York entry time. | Dashboard refreshes without controlling the bot; recommendations reproduce stored checkpoint inputs and JSONL remains the canonical audit log. |
