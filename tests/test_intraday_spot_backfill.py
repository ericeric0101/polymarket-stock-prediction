...
def test_backfill_writes_one_resumable_file_per_nvda_tsla_market(monkeypatch, tmp_path: Path) -> None:
    ...
    report = backfill_pyth_intraday_spots(
        discovery_path=discovery, output_dir=output, api_key="secret", pause_seconds=0
    )

    assert (report.requested, report.completed, report.skipped, report.failed) == (2, 2, 0, 0)
    assert (output / "1_NVDA_2026-04-27_pyth_intraday.csv").read_text(encoding="utf-8").splitlines() == [
        "DateTime,Spot",
        "2026-04-27T13:30:00+00:00,100.0",
    ]
    resumed = backfill_pyth_intraday_spots(
        discovery_path=discovery, output_dir=output, api_key="secret", pause_seconds=0
    )
