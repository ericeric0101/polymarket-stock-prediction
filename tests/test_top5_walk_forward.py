...
class TopFiveWalkForwardTests(unittest.TestCase):
    def test_daily_cap_and_one_entry_per_market_are_enforced(self) -> None:
        observations = [_observation(str(index), "2026-07-20", "1200_EDT", 0.90, "UP") for index in range(7)] + [
            _observation("0", "2026-07-20", "1400_EDT", 0.95, "UP"),
        ]
        result = run_top_five_policy(
            observations,
            policy=TopFivePolicy(("1200_EDT", "1400_EDT"), buffer=0.01, minimum_edge=0.02, max_daily_entries=5),
        )
    ...
    def test_training_calibrator_leaves_sparse_bands_raw_and_shrinks_supported_bands(self) -> None:
        sparse = TrainingProbabilityCalibrator.fit((_observation("one", "2026-07-20", "1200_EDT", 0.85, "DOWN"),))
        self.assertEqual(sparse.transform(0.85), 0.85)
        supported = TrainingProbabilityCalibrator.fit(
            tuple(_observation(str(index), f"2026-07-{20 + index:02d}", "1200_EDT", 0.85, "DOWN") for index in range(5))
        )
