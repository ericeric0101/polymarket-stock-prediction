...
def _position(
    *, status: str, outcome: str = "UP", settlement_outcome: str | None = None, pnl: float | None = None
) -> PaperPosition:
    return PaperPosition(
        ...
        model_version="test",
        settled_at=datetime(2026, 7, 20, tzinfo=UTC) if status == "SETTLED" else None,
        settlement_outcome=settlement_outcome,
        payout=1 if outcome == settlement_outcome else 0 if status == "SETTLED" else None,
        realized_pnl=pnl,
    )
