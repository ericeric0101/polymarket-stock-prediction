...
def test_execution_observation_persists_book_fee_and_signal_link(tmp_path: Path) -> None:
    ...
    journal.record_execution_observation(
        observed_at=datetime(2026, 7, 27, 16, tzinfo=UTC), signal_id="position-1",
        observation_kind="MARKOUT_300S", market_id="market-1", symbol="TSLA", outcome="UP", token_id="token-up",
        spot=320.0, price_to_beat=318.5, fair_probability=0.72, best_bid=0.66, best_ask=0.68, fee_rate=0.04,
    ...
    row = connection.execute("SELECT signal_id, observation_kind, spot, price_to_beat, best_bid, best_ask, fee_rate, book_payload_json FROM execution_observations").fetchone()
    connection.close()
