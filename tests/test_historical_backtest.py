...
from polymarket_stock.historical_backtest import UnderlyingSpotPoint, close_risk_profile, price_gap_from_daily_closes, replay_daily_up_down_market
...
class HistoricalBacktestTests(unittest.TestCase):
    ...
    def test_close_risk_profile_tracks_late_probability_moves(self) -> None:
        ...
        profile = close_risk_profile(
            resolves_at=resolves_at, up_history=points, price_to_beat=100, final_price=101,
    ...
    def test_replay_selects_positive_edge_historical_trades(self) -> None:
        ...
        report = replay_daily_up_down_market(
            candidate=_candidate(), symbol="TSLA", resolves_at=resolves_at,
            closes_before_market=closes, final_close=DailyClose("2026-07-27", 110),
            up_history=up_history, down_history=down_history, settlement=settlement, spot_history=spot_history,
            minimum_edge=0.0, model_error_buffer=0.0,
        )
    ...
    def test_replay_without_spot_history_only_reports_gap_and_close_risk(self) -> None:
        ...
        report = replay_daily_up_down_market(
            candidate=_candidate(), symbol="TSLA", resolves_at=resolves_at,
            closes_before_market=closes, final_close=DailyClose("2026-07-27", 110),
            up_history=up_history, down_history=down_history,
            settlement=MarketSettlement("market-1", True, "UP", {"closed": True}),
            minimum_edge=0.0, model_error_buffer=0.0,
        )
