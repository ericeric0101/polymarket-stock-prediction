...
def test_batch_replay_uses_only_prior_days_and_reports_walk_forward_shortage(tmp_path: Path) -> None:
    ...
    for index in range(22):
        ...
        reference = {
            "market_id": str(index), "symbol": "TSLA", "market_day": day,
        ...
        (tmp_path / f"{stem}_up_clob.csv").write_text("DateTime,Price\n" + f"{day}T17:00:00+00:00,0.40\n", encoding="utf-8")
        (tmp_path / f"{stem}_down_clob.csv").write_text("DateTime,Price\n" + f"{day}T17:00:00+00:00,0.60\n", encoding="utf-8")
        (tmp_path / f"{stem}_pyth_intraday.csv").write_text("DateTime,Spot\n" + f"{day}T17:00:00+00:00,{101 + index * 0.1}\n", encoding="utf-8")

    report = run_pyth_clob_backtest(
        data_dir=tmp_path, buffers=(0.01, 0.02), minimum_edge=0.0,
        training_days=20, validation_days=5, minimum_training_trades=1,
    )
