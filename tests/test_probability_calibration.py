...
def _observation(
    ...
    *,
    ...
    won: bool = True,
    direction: str = "UP",
    regime: str = "REALIZED_VOL_FALLBACK",
    ...
    return FirstSignalCalibrationObservation(
        market_id=f"market-{days}-{index}",
        symbol="TSLA",
        evaluated_at=evaluated_at,
        model_outcome=direction,
        selected_fair_probability=probability,
        entry_ask=0.5,
        entry_fee=0.01,
        winning_outcome=direction if won else ("DOWN" if direction == "UP" else "UP"),
        model_version="test-v1",
        option_iv_status="IV_VALID" if regime == "IV_VALID" else "IV_UNAVAILABLE",
        iv_regime=regime,
        spot_provider="PYTH_HERMES",
        threshold_distance_bps=42.0,
    )
