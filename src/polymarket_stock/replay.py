...
def replay_settled_positions(positions: Iterable[PaperPosition]) -> ReplayReport:
    ...
    settled = tuple(position for position in all_positions if position.status == "SETTLED" and position.settlement_outcome)
    if not settled:
        return ReplayReport(
            0, sum(position.status == "OPEN" for position in all_positions), 0.0, None, None, None,
    ...
    return ReplayReport(
        len(settled), sum(position.status == "OPEN" for position in all_positions),
        sum(position.realized_pnl or 0 for position in settled), metrics.brier_score, metrics.log_loss,
        sum(edges) / len(edges), skipped_excluded_positions,
    )
...
def replay_market_observations(observations: Iterable[ReplayObservation]) -> ReplayReport:
    ...
    metrics = calibration_metrics(predictions)
    edges = [
        (item.fair_up_probability - item.up_ask) if item.up_ask is not None else
        ((1 - item.fair_up_probability) - item.down_ask) if item.down_ask is not None else 0.0
        for item in items
    ]
