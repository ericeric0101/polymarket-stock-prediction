...
class AboveXBacktestReport:
    ...
    def as_payload(self) -> Mapping[str, object]:
        return {
            "market_type": "ABOVE_X", "source": self.source,
            "execution_price_assumption": self.execution_price_assumption,
            "contract_count": self.contract_count, "observation_count": self.observation_count,
            "trade_count": self.trade_count, "wins": self.wins,
            "win_rate": self.wins / self.trade_count if self.trade_count else None,
            "total_pnl": round(self.total_pnl, 8), "brier_score": self.brier_score,
            "log_loss": self.log_loss, "skipped_contracts": self.skipped_contracts,
...
def run_above_x_backtest(
    *, discovery_path: Path, data_dir: Path, spot_data_dir: Path,
    minimum_edge: float = 0.02, lookback_days: int = 20,
    ...
    for contract in contracts:
        ...
        yes_path, no_path, settlement_path = (
            data_dir / f"{stem}_yes_clob.csv", data_dir / f"{stem}_no_clob.csv",
            data_dir / f"{stem}_settlement.json",
        )
        spot_path = next(iter(sorted(spot_data_dir.glob(f"*_{contract.symbol}_{contract.market_date}_pyth_intraday.csv"))), None)
        ...
        yes = _read_prices(yes_path); no = _read_prices(no_path); spots = _read_spots(spot_path)
        for name in checkpoints:
            evaluated_at = datetime.combine(datetime.fromisoformat(contract.market_date).date(), CHECKPOINTS[name], tzinfo=NEW_YORK).astimezone(UTC)
            yes_price = _latest(yes, evaluated_at); no_price = _latest(no, evaluated_at); spot = _latest(spots, evaluated_at)
            if yes_price is None or no_price is None or spot is None or contract.resolves_at <= evaluated_at:
                continue
            fair = digital_up_probability(
                spot=spot, threshold=contract.strike, annual_volatility=volatility,
    ...
    brier = sum((prob - outcome) ** 2 for prob, outcome in probabilities) / len(probabilities) if probabilities else None
    loss = sum(-(outcome * log(min(max(prob, 1e-9), 1 - 1e-9)) + (1 - outcome) * log(1 - min(max(prob, 1e-9), 1 - 1e-9))) for prob, outcome in probabilities) / len(probabilities) if probabilities else None
    return AboveXBacktestReport(
        ...
        contract_count=len(contracts), observation_count=observations, trade_count=trades, wins=wins,
        total_pnl=total_pnl, brier_score=brier, log_loss=loss, skipped_contracts=skipped,
        checkpoints=checkpoints,
    )


def _contract(item: Mapping[str, object]) -> _Contract:
    return _Contract(str(item["market_id"]), str(item["symbol"]).upper(), str(item["market_date"]), float(item["strike"]), datetime.fromisoformat(str(item["resolves_at"]).replace("Z", "+00:00")).astimezone(UTC))
...
def _daily_closes(data_dir: Path) -> dict[str, tuple[DailyClose, ...]]:
    ...
    for path in data_dir.glob("*_pyth_references.json"):
        try:
            payload = json.loads(path.read_text(encoding="utf-8")); symbol = str(payload["symbol"]).upper()
            result.setdefault(symbol, []).append(DailyClose(str(payload["market_day"]), float(payload["final_price"]["price"])))
